...
if version_lt "${cmake_version}" "3.15"; then
  echo "cmake version required 3.15"
  exit 0
fi
...
BuildJsonCPP() {
  ...
  cmake -S "${jsoncpp_dir}" -B "${jsoncpp_dir}/build" -DCMAKE_AR="${AR}" -DCMAKE_RANLIB="${RANLIB}" -DCMAKE_LINKER="${LINK}" -DCMAKE_SYSTEM_PROCESSOR="${CMAKE_SYSTEM_PROCESSOR}" -DCMAKE_SYSTEM_NAME="${CMAKE_SYSTEM_NAME}" -DJSONCPP_WITH_POST_BUILD_UNITTEST=0 -DCMAKE_CXX_FLAGS=-fPIC -DBUILD_STATIC_LIBS=ON -DBUILD_SHARED_LIBS=OFF -DCMAKE_INSTALL_PREFIX="${install_lib_dir}"
...
BuildRocketMQClient() {
  ...
  if [ $test -eq 0 ]; then
    cmake -S "${basepath}" -B "${build_dir}" -DCMAKE_AR="${AR}" -DCMAKE_RANLIB="${RANLIB}" -DCMAKE_LINKER="${LINK}" -DCMAKE_SYSTEM_PROCESSOR="${CMAKE_SYSTEM_PROCESSOR}" -DCMAKE_SYSTEM_NAME="${CMAKE_SYSTEM_NAME}" -DLibevent_USE_STATIC_LIBS=ON -DJSONCPP_USE_STATIC_LIBS=ON -DZLIB_USE_STATIC_LIBS=ON -DBUILD_ROCKETMQ_STATIC=ON -DBUILD_ROCKETMQ_SHARED=OFF
  else
    if [ $codecov -eq 1 ]; then
      cmake -S "${basepath}" -B "${build_dir}" -DCMAKE_AR="${AR}" -DCMAKE_RANLIB="${RANLIB}" -DCMAKE_LINKER="${LINK}" -DCMAKE_SYSTEM_PROCESSOR="${CMAKE_SYSTEM_PROCESSOR}" -DCMAKE_SYSTEM_NAME="${CMAKE_SYSTEM_NAME}" -DLibevent_USE_STATIC_LIBS=ON -DJSONCPP_USE_STATIC_LIBS=ON -DZLIB_USE_STATIC_LIBS=ON -DBUILD_ROCKETMQ_STATIC=ON -DBUILD_ROCKETMQ_SHARED=OFF -DRUN_UNIT_TEST=ON -DCODE_COVERAGE=ON
    else
      cmake -S "${basepath}" -B "${build_dir}" -DCMAKE_AR="${AR}" -DCMAKE_RANLIB="${RANLIB}" -DCMAKE_LINKER="${LINK}" -DCMAKE_SYSTEM_PROCESSOR="${CMAKE_SYSTEM_PROCESSOR}" -DCMAKE_SYSTEM_NAME="${CMAKE_SYSTEM_NAME}" -DLibevent_USE_STATIC_LIBS=ON -DJSONCPP_USE_STATIC_LIBS=ON -DZLIB_USE_STATIC_LIBS=ON -DBUILD_ROCKETMQ_STATIC=ON -DBUILD_ROCKETMQ_SHARED=OFF -DRUN_UNIT_TEST=ON
    fi
  fi
...
BuildGoogleTest() {
  ...
  cmake -S "${gtest_dir}" -B "${gtest_dir}/build" -DCMAKE_AR="${AR}" -DCMAKE_RANLIB="${RANLIB}" -DCMAKE_LINKER="${LINK}" -DCMAKE_CXX_FLAGS=-fPIC -DBUILD_STATIC_LIBS=ON -DBUILD_SHARED_LIBS=OFF -DCMAKE_INSTALL_PREFIX="${install_lib_dir}"

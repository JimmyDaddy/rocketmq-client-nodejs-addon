...
class RocketMQPushConsumer extends EventEmitter {
    ...
    [START_OR_SHUTDOWN](method, callback) {
        ...
        this.core[method]((err, ret) => {
            ...
            if(method === "start") {
                ...
                if(!consumerRef) timer = setInterval(function() {}, 24 * 3600 * 1000);
                consumerRef++;
            } else {
                this.status = START_STATUS.STOPPED;
                consumerRef--;
                if(!consumerRef) clearInterval(timer);
            }
    ...
    subscribe(topic, expression) {
        assert(this.status === START_STATUS.STOPPED, "Consumer must be in STOPPED state to subscribe");
        assert(this.status !== START_STATUS.STARTING, "Consumer cannot be in STARTING state");
        assert(this.status !== START_STATUS.STOPPING, "Consumer cannot be in STOPPING state");
        assert(topic && typeof topic === "string", "Topic must be a non-empty string");
        assert(!expression || expression && typeof expression === "string", "Expression must be a string if provided");
